		...
		SWITCH '(' expr ')' '{' caseblock '}'
				{
					...
					label = "NEXT";
					replace (s1, "LAST", "NEXT");
					ret = (char*)malloc(strlen($3->code)+strlen($6)+100);
					ret[0]=0;
					strcat(ret,$3->code);strcat(ret,"\n");strcat(ret,$6);
					$$ = ret;
				}
				;

caseblock	:  CASE expr ':' '{' block BREAK ';' '}' caseblock
			{
				label=newLabel(); label2 = newLabel();
				ret = (char*)malloc(strlen($9)+100); 
				...
				strcat(ret,"\n"); strcat(ret,label); strcat(ret," : \n"); strcat(ret,$5); strcat(ret,"\n"); strcat(ret,"jump NEXT"); strcat(ret,"\n");strcat(ret,label2); strcat(ret," : "); strcat(ret,$9); 
				$$=ret;
			}
			|
			CASE expr  ':' block
